...
const BOOTSTRAP = [
  ...
  "}",
  "function row(label,a,b){return '<div style=\"display:flex;justify-content:space-between;margin-top:3px\"><span>'+a+'</span><span style=\"color:#8b949e\">'+label+'</span><span>'+b+'</span></div>';}",
  "window.__sb={",
  ...
  "    if(d.mode==='today'){s.textContent='Today';var g=d.games||[];dt.textContent=g.length+' matches';if(btn)btn.style.display='none';if(out)out.innerHTML='';var h='';for(var i=0;i<g.length;i++){h+='<div style=\"display:flex;justify-content:space-between;padding:2px 0\"><span>'+g[i].home+' '+g[i].hs+' – '+g[i].as+' '+g[i].away+'</span><span style=\"color:#8b949e;font-size:11px\">'+(g[i].detail||'')+'</span></div>';}st.innerHTML=h||'<span style=\"color:#8b949e\">no matches today</span>';return;}",
  ...
  "  result:function(d){mk();var o=document.getElementById('sb-h2h-out'),b=document.getElementById('sb-h2h');if(b)b.textContent='Head-to-head';if(!o)return;var g=(d&&d.games)||[];var h='<div style=\"color:#8b949e;font-size:11px;margin-bottom:4px\">Past meetings — '+((d&&d.team)||'')+'</div>';if(!g.length)h+='<div style=\"color:#8b949e\">none found</div>';for(var i=0;i<g.length;i++){var col=g[i].result==='W'?'#3fb950':g[i].result==='L'?'#f85149':'#8b949e';h+='<div style=\"display:flex;gap:8px;padding:2px 0;border-bottom:1px solid #21262d\"><span style=\"color:#8b949e;width:5.5rem\">'+g[i].date+'</span><b style=\"width:2.5rem\">'+g[i].score+'</b><span style=\"color:'+col+'\">'+g[i].result+'</span></div>';}o.innerHTML=h;}",
  "};",
  "mk();",
  "})();",
].join("\n");
